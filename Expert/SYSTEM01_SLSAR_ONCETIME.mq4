...
void OnTick() {
   ...
   static int lastYear = TimeYear(iTime(Symbol(),PERIOD_CURRENT,0));
   int currentYear = TimeYear(iTime(Symbol(),PERIOD_CURRENT,0));
  ...
  //CHECK BUY SELL \\
   ...
   if(D1 != iTime(Symbol(),0,0)) {
      // START NORMAL ORDER \\
      //BUY ORDER\\
      if(signal_order == "buy" && buy_once == true && OrdersTotal() == 0 && OrdersTotal() == 0 && lastYear == currentYear) {
         ticket1 = OrderSend(_Symbol,OP_BUY,Lots_Buy,Ask,3,Stoploss_Psar,Ask + tp_buy,NULL,0,0,clrGreen);
         buy_once = false;
      }
      //SELL ORDER\\
      if(signal_order == "sell" && sell_once == true && OrdersTotal() == 0 && OrdersTotal() == 0 && lastYear == currentYear) {
         ...
         if(OrderSymbol() == Symbol()) {
            if(OrderType() == OP_BUY) {
               if(OrderStopLoss() < last_psar) {
                  bool modify = OrderModify(OrderTicket(),OrderOpenPrice(),last_psar,OrderTakeProfit(),0,CLR_NONE);
                  if(modify == false) Print("Traling Stop Buy : ",GetLastError());
               }
            }
         }
      }
      ...
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) {
         if(OrderSymbol() == Symbol()) {
            if(OrderType() == OP_SELL) {
               if(OrderStopLoss() > last_psar) {
                  bool modify = OrderModify(OrderTicket(),OrderOpenPrice(),last_psar,OrderTakeProfit(),0,CLR_NONE);
                  if(modify == false) Print("Traling Stop Sell : ",GetLastError());
               }
            }
         }
      }
...
void closeOrderBuy() {
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) {
         if(OrderSymbol() == Symbol()) {
            if(OrderType() == OP_BUY) {
               bool closeOrder = OrderClose(OrderTicket(),OrderLots(),MarketInfo(OrderSymbol(),MODE_BID),5,clrNONE);
               if(closeOrder == false) Print("Close Order Buy : ",GetLastError());
            }
         }
      }
   }
...
void closeOrderSell() {
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) {
         if(OrderSymbol() == Symbol()) {
            if(OrderType() == OP_SELL) {
               bool closeOrder = OrderClose(OrderTicket(),OrderLots(),MarketInfo(OrderSymbol(),MODE_ASK),5,clrNONE);
               if(closeOrder == false) Print("Close Order Sell : ",GetLastError());
            }
         }
      }
   }
