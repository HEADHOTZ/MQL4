   ...
   for(int i = 0; i < uncalculatedBar; i++)
     {
      ...
      double openPrice  = iOpen(Symbol(),PERIOD_CURRENT,DAY);

      double val_spread = (MarketInfo(Symbol(),MODE_SPREAD)) * _Point;

      double diffBuy = (close2Price - psar2) / _Point;
      double diffSell = ((psar2 - close2Price) / _Point);
      double tpBuy = (close2Price - psar2) * TP;
      double tpSell = ((psar2 - close2Price) * TP);
      ...
      if(Symbol() == "XAUUSDc")
        {
         lotBuy = risk / ((close2Price - psar2) * 100);
         lotSell = risk / ((psar2 - close2Price) * 100);
        }

      else
         if(Symbol() == "XAGUSDc")
           {
            lotBuy = risk / ((close2Price - psar2) * 5000);
            lotSell = risk / ((psar2 - close2Price) * 5000);
           }
      ...
      // Buy condition
      if(close2Price > psar2)
        {
         ENBuffer[i] = openPrice;
         SLBuffer[i] = psar2 - spreadSL;
         //TPBuffer[i] = ((ENBuffer[i] - SLBuffer[i]) * TP) + ENBuffer[i];
         TPBuffer[i] = ((ENBuffer[i] - (psar2)) * TP) + ENBuffer[i];
         ...
         if(close2Price < psar2)
           {
            ...
            TPBuffer[i] = ENBuffer[i] - (((psar2) - ENBuffer[i]) * TP);
